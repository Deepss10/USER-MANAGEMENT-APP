...
public class UserDao {
	...
	private static final String INSERT_USER = "insert into users(name,email,country,age) values(?,?,?,?)";

	private static final String UPDATE_USER = "update users set name=?,email=?,country=?,age=? where id=?";
	...
	public User getUserById(int id) {
		...
		try {
			...
			rs.next();
			user = new User(rs.getInt("id"), rs.getString("name"), rs.getString("email"), rs.getString("country"), rs.getInt("age"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return user;
